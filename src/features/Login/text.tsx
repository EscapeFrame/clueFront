...
function Oauth2Test() {
  ...
  useEffect(() => {
    if (!accessToken) {
      (async () => {
        try {
          const res = await fetch("http://10.129.59.166:8080/refresh-token", {
            method: "POST",
            credentials: "include",
          });
  ...
  useEffect(() => {
    if (window.location.pathname === "/register") {
      fetch("http://10.129.59.166:8080/first-register", {
        method: "POST",
        credentials: "include",
      })
        .then((res) => {
  ...
  const handleRegisterSubmit = async (e:any) => {
    ...
    try {
      ...
      const res = await fetch("http://10.129.59.166:8080/register", {
        method: "POST",
        credentials: "include",
  ...
  // 4️⃣ 소셜 로그인 핸들러
  const onNaverLogin = () => {
    window.location.href = "http://10.129.59.166:8080/oauth2/authorization/naver";
  };
  const onGoogleLogin = () => {
    window.location.href = "http://10.129.59.166:8080/oauth2/authorization/google?prompt=login";
  };

  // 5️⃣ 보호된 리소스 호출
  const getData = async () => {
    try {
      const res = await fetch("http://10.129.59.166:8080/my", {
        method: "GET",
        credentials: "include",
        headers: {
  ...
  const onLogout = async () => {
    try {
      const res = await fetch("http://10.129.59.166:8080/logout", {
        method: "POST",
        credentials: "include",
        headers: {
