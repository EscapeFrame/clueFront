...
export function TAssignmentCard({ data, onCheck }: AssignmentCardProps) {
  ...
  const [description, setDescription] = useState(data.description || "");  // description 상태 추가
  ...
  return (
    <>
      ...
      <SlidePanel
        ...
        description={description}  // 상태로 넘김
        teacherFiles={
          data.hasFile
            ? [{ id: crypto.randomUUID(), name: data.fileName!, size: data.fileSize! }]
            : []
        ...
        onDescriptionUpdate={(newDesc) => {
          setDescription(newDesc);  // 로컬 상태 업데이트
          updateAssignmentDescription(newDesc);  // 서버 저장 요청
        }}
      />
    </>
  );
